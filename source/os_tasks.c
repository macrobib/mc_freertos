...
typedef struct tskTaskControlBlock
{
	...
	char				pcTaskName[ configMAX_TASK_NAME_LEN ];/*< Descriptive name given to the task when created.  Facilitates debugging only. */ /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
		...
		/*Added to support the use case of MC System. First variable is to keep track of the budget
		 * and the second is to mark the task if it is discarded due to criticality change.
		 * */
...
PRIVILEGED_DATA static eSystemCriticality pxSystemCrit = eLOW;
PRIVILEGED_DATA static List_t pxLowCritReadyTasksList; /*In lower criticality mode, tasks to this queue*/
...
/*
 * Set xNextTaskUnblockTime to the time at which the next BlockexTaskGetTickCoutd state task
...
BaseType_t xTaskGenericCreate( TaskFunction_t pxTaskCode, const char * const pcName, const uint16_t usStackDepth, void * const pvParameters,
						...
						StackType_t * const puxStackBuffer, const MemoryRegion_t * const xRegions, const MCParam_t* pxMCParam) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
...
StackType_t *pxTopOfStack;
	...
	if( pxNewTCB != NULL )
	{
		...
		prvInitialiseMCVariables(pxNewTCB, pxMCParam);
...
BaseType_t xReturn;
	...
	#if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
	{
		...
		xReturn = xTaskCreate( prvIdleTask, "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle, NULL ); /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
	}
	#else
	{
		/* Create the idle task without storing its handle. */
		xReturn = xTaskCreate( prvIdleTask, "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL, NULL );  /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
	}
...
BaseType_t xSwitchRequired = pdFALSE;
			...
			if( xConstTickCount >= xNextTaskUnblockTime )
			{
				for( ;; )
				{
					...
					else
					{
						...
						list.
						* If the system is in high criticality mode, move the low crit tasks to the low crit ready list.
						*/
						prvAddTaskToReadyList( pxTCB );
	...
	else
	{
		...
		/*Check for a overrun condition and handle criticality change.
		 * */
		prvCheckAndHandleCriticalityChange();
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	for( ;; )
	{
		/*Check the criticality level and if in high critical state, schedule the tasks in low critical pending list and do the clean up if necessary.
		 * */
		prvUseIdleBudget();
...
static void prvInitialiseMCVariables(TCB_t* const pxTCB, const MCParam_t* pxMCData){
	...
	if(pxMCData == NULL){
		/*Task initialized for idle condition, should run at lowest priority with no inhibitions when it does.*/
	...
	if(prvGetSystemCrit() == eLOW){
		...
		/**Undefined state: initiate a trace for the given condition.**/
	}
}

void uxListReinitialize(List_t* pxList){
	/*Need to find the task boundary and reset.*/
	TCB_t* pTask = NULL;
	const ListItem_t* pxLastItem = listGET_END_MARKER(pxList);
	const ListItem_t* pxCurrItem = pxList->pxIndex;
	UBaseType_t val = 0;
	UBaseType_t timeToWake = 0;
	while(pxCurrItem != pxLastItem){
		pTask = ( TCB_t * ) listGET_LIST_ITEM_OWNER( pxCurrItem );
		if(NULL != pTask){
			/*Replenish the budget and insert the tasks back to waiting queue to be released at next period.*/
			if(pTask->uxPriority != 0){/*Prevent inserting idle task to runqueue again.*/
				val = xTaskGetTickCount() - (xTaskGetTickCount() % (pTask->pxMCParams->xPeriod));
				pTask->xBudgetRemaining = pTask->pxMCParams->xBudgetLow;
				pTask->xNextDeadline = val + pTask->pxMCParams->xPeriod + pTask->pxMCParams->xDeadline;
				timeToWake = val + pTask->pxMCParams->xPeriod;
				listSET_LIST_ITEM_VALUE( &( pTask->xGenericListItem ), timeToWake );
				vListInsert( pxDelayedTaskList, &( pTask->xGenericListItem ) );
				/*TODO: Handle the possible overrun of timetowake up variable.*/
			}
		}
		pxCurrItem = listGET_NEXT(pxCurrItem);
	}
}
 ...
 * Also do the clean up of the tasks that have expired deadline and re-arm them if necessary.
 * */
void prvManageLowCritTasks(void){
	TCB_t* pTask = NULL;
	ListItem_t *pxListItem;
	ListItem_t const *pxListEnd;
	UBaseType_t index;
	/*Brute force through entire process list
	 * TODO: Move to an amortized movement of tasks to low crit queue.
	 * */
	for(index = 1; index < configMAX_PRIORITIES; index++){
		...
		while(pxListItem != pxListEnd){
			/*Iterate to the end of the list and re-arm/pick task accordingly.*/
			pTask = ( TCB_t * ) listGET_LIST_ITEM_OWNER( pxListItem );
			pxListItem = listGET_NEXT( pxListItem );
			if(pTask->pxMCParams->xBudgetHigh == 0){
				uxListRemove(&( pTask->xGenericListItem ));
				vListInsert( &pxLowCritReadyTasksList, &(pTask->xGenericListItem) );
			}
		}
	}
}

/**Utilizing the idle budget or transition back to low crit.**/
void prvUseIdleBudget(void){
	if(xUseIdleBudget){
		prvScheduleLowCritTasks();
	}
	else{
		prvTransitionToLowCrit();
	}
}

/*Get the next release value to push it to the delayed list.*/
void prvSetNextRelease(TCB_t* ptask){

	/*Task may have been pushed down for too long, push it to appropriate range.*/
	while((ptask->pxMCParams->xLastActivation + ptask->pxMCParams->xPeriod) < xTickCount){
		ptask->pxMCParams->xLastActivation = ptask->pxMCParams->xLastActivation + ptask->pxMCParams->xPeriod;
	}

	ptask->xNextDeadline = ptask->pxMCParams->xLastActivation + ptask->pxMCParams->xPeriod + ptask->pxMCParams->xDeadline;
	if(pxSystemCrit == eLOW){
		ptask->xBudgetRemaining = ptask->pxMCParams->xBudgetLow;
	}
	else{
		ptask->xBudgetRemaining = ptask->pxMCParams->xBudgetHigh;
	}
}

/*If background scheduling enabled in low-crit, handle here*/
void prvScheduleLowCritTasks(void){
	UBaseType_t index;
	TCB_t* pTask = NULL;
	ListItem_t *pxListItem;
	ListItem_t const *pxListEnd;
	for(index=1; index< configMAX_PRIORITIES; index++){
		...
		while(pxListItem != pxListEnd){
			pTask = ( TCB_t * ) listGET_LIST_ITEM_OWNER( pxListItem );
			pxListItem = listGET_NEXT( pxListItem);
			/*If deadline expired, update budget and move to the delayed list.*/
			if(pTask->xNextDeadline <= xTickCount){
				prvSetNextRelease(pTask);
				listSET_LIST_ITEM_VALUE( &( pTask->xGenericListItem ),
						(pTask->pxMCParams->xLastActivation + pTask->pxMCParams->xPeriod));
				vListInsert( pxDelayedTaskList, &( pTask->xGenericListItem ));
			}
			else{
				/*Schedule and break from lookup*/
				xUseIdleBudget = pdTRUE;
				pxCurrentTCB = pTask;
				break;
			}
		}
...
/*Transitioning back to low critical state. Re arm the low crit tasks and move to the main queue.*/
void prvTransitionToLowCrit(void){
	UBaseType_t index;
	UBaseType_t delta = 0;
	TCB_t* pTask = NULL;
	ListItem_t *pxListItem;
	...
	for(index=1; index< configMAX_PRIORITIES; index++){
		...
		while(pxListItem != pxListEnd){
			pTask = ( TCB_t * ) listGET_LIST_ITEM_OWNER( pxListItem );
			pxListItem = listGET_NEXT( pxListItem );
			delta = pTask->pxMCParams->xBudgetHigh - pTask->pxMCParams->xBudgetLow;
			if(delta > pTask->xBudgetRemaining){
				pTask->xBudgetRemaining = pTask->xBudgetRemaining - delta;
			}
			else{
				/*TODO: verify if this step is necessary as the high crit tasks are penalized here.
				 * */
				pTask->xBudgetRemaining = 0;
				uxListRemove(&(pTask->xGenericListItem));
				vListInsert(pxDelayedTaskList, &(pTask->xGenericListItem));
			}
		}
...
void prvCheckAndHandleCriticalityChange(void){
	...
	if(pxSystemCrit == eLOW){
			...
			if(pxCurrentTCB->pxMCParams->xBudgetHigh == 0){
				...
				prvAddCurrentTaskToDelayedList(pxCurrentTCB->pxMCParams->xLastActivation
						+ pxCurrentTCB->pxMCParams->xPeriod);
			}
			else{
				/*Change system criticality.*/
				if(!xToleranceCount){
					pxSystemCrit = eHIGH;
					prvTransitionToHighCrit();
				}
				else{
					xToleranceCount--;
				}
		...
		if(pxCurrentTCB->xBudgetRemaining <= 0){
			/*Schedule error detected, budget overrun in higher criticality.*/
		}
	}
}
#endif
