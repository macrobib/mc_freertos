...
struct xPARAM_MC{
	TickType_t xBudgetLow;
	TickType_t xBudgetHigh;
	TickType_t xPeriod;
	TickType_t xDeadline;
	TickType_t xLastActivation;
	TickType_t xPriority;
};
typedef struct xPARAM_MC MCParam_t; /*Store the statically defined task variables.*/
 ...
 * \ingroup Tasks
 */
#define xTaskCreate( pvTaskCode, pcName, usStackDepth, pvParameters, uxPriority, pxCreatedTask, pMCDef ) xTaskGenericCreate( ( pvTaskCode ), ( pcName ), ( usStackDepth ), ( pvParameters ), ( uxPriority ), ( pxCreatedTask ), ( NULL ), ( NULL ), (pMCDef) )
...
BaseType_t xTaskGenericCreate( TaskFunction_t pxTaskCode, const char * const pcName,const uint16_t usStackDepth,
		...
		StackType_t * const puxStackBuffer,const MemoryRegion_t * const xRegions, const MCParam_t* const pxMCParam ) PRIVILEGED_FUNCTION; /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
